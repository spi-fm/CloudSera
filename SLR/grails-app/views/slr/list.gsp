    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
          ...
          <div style="margin-top: 5px; margin-bottom: 20px;">
            ...
            <tbody>
                <g:each in="${listOfSlr}" var="eachSLR">
                  <tr class="gradeX">
                    <td><g:link controller="slr" action="show" params="[guidSlr: "${eachSLR.guid}"]">${eachSLR.title}</g:link></td>
                    <td>
                      <g:if test="${eachSLR.state.equals("fase1")}">
                        Review Planning
                      </g:if>
                      <g:if test="${eachSLR.state.equals("fase2")}">
                        Review Execution
                      </g:if>
                      <g:if test="${eachSLR.state.equals("fase3")}">
                        Export Results
                      </g:if>
                    </td>
                    <td>${formatDate(format: 'dd/MMM/yyyy', date: eachSLR.submitDate)}</td>
                    <td>
                      <g:if test="${eachSLR.searchs.size() > 0}">
                        <g:link controller="slr" action="searchs" params="[guid: "${eachSLR.guid}"]">${eachSLR.searchs.size()} Searches</g:link>
                      </g:if>
                      <g:else>
                        ${eachSLR.searchs.size()} Searches
                      </g:else>
                    </td>
                    <td>
                      <g:if test="${eachSLR.totalReferences > 0}">
                        <g:link controller="slr" action="references" params="[guid: "${eachSLR.guid}"]">${eachSLR.totalReferences} References</g:link>
                      </g:if>
                      <g:else>
                        ${eachSLR.totalReferences} References
                      </g:else>
                    </td>
                    <td>
              				<g:link title="Research Questions" type="button" class="btn btn-outline btn-primary btn-circle" controller="slr" action="researchQuestions" params="[guid: "${eachSLR.guid}"]"><i class="fa fa-question"></i></g:link>
              				<g:link title="Criterions" class="btn btn-outline btn-primary btn-circle" controller="slr" action="criterions" params="[guid: "${eachSLR.guid}"]"><i class="fa fa-bookmark"></i></g:link>
              				<g:link title="Specific Attributes" class="btn btn-outline btn-primary btn-circle" controller="slr" action="specAttributes" params="[guid: "${eachSLR.guid}"]"><i class="glyphicon glyphicon-tags"></i></g:link>
              				<g:link title="Searchs" type="button" class="btn btn-outline btn-primary btn-circle" controller="slr" action="searchs" params="[guid: "${eachSLR.guid}"]"><i class="glyphicon glyphicon-search"></i></g:link>
              				<g:if test="${eachSLR.noDrop == false}">
              					<button title="Delete Review" type="button" class="btn btn-outline btn-danger btn-circle" data-toggle="modal" data-target="#myModalDrop" onclick="getIdSlr('${eachSLR.guid}')"><i class="fa fa-times"></i></button>
              				</g:if>
              				<p> </p>
              				<g:link title="Export Excel" type="button" class="btn btn-outline btn-success btn-circle" controller="slr" action="exportToExcel" params="[guid: "${eachSLR.guid}"]"><i class="fa fa-file-excel-o"></i></g:link>
              				<g:link title="Export Word" type="button" class="btn btn-outline btn-success btn-circle" controller="slr" action="exportToWord" params="[guid: "${eachSLR.guid}"]"><i class="fa fa-file-word-o"></i></g:link>
              				<g:link title="Export Bibtex" type="button" class="btn btn-outline btn-success btn-circle" controller="slr" action="exportToBibTex" params="[guid: "${eachSLR.guid}"]"><i class="fa fa-file-code-o"></i></g:link>
              				<g:link title="Graphs" type="button" class="btn btn-outline btn-success btn-circle" controller="slr" action="graphs" params="[guid: "${eachSLR.guid}"]"><i class="glyphicon glyphicon-stats"></i></g:link>
                    </td>
                    <td>
                      <g:if test="${eachSLR.visibility_id==1}">PUBLISEHD</g:if>
                      <g:elseif test="${eachSLR.visibility_id==2}">PUBLIC</g:elseif>
                      <g:elseif test="${eachSLR.visibility_id==3}">PRIVATE</g:elseif>
                      <g:elseif test="${eachSLR.visibility_id==4}">DRAFT</g:elseif>
                      <g:else>?????</g:else>
                    </td>
                  </tr>
                </g:each>
