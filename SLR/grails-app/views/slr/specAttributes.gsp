    	...
    	<div class="modal-dialog">
	    	<div class="modal-content">
	    		<div class="modal-header">
      				...
      				<h4 class="modal-title" id="myModalLabel">Create Specific Attribute</h4>
			    </div>
		    	<g:form controller="specificAttribute" action="save" method="POST" name="myFormAttribute" id="myFormAttribute">
		    		...
		    		<div class="modal-body">
				    	...
				    	<div class="form-inline">
				    		<table>
				    			...
				    			<tr>
				    				<td><label id="lbOptions" style="visibility: hidden;"><b>Options: (;)</b></label></td>
				    				<td>
				    ...
				    <div class="modal-footer">
						...
						<g:submitButton name="create" class="btn btn-primary" value="Create attribute"/>
					</div>
				</g:form>
		    </div>
		</div>
	...
	<div class="modal fade" id="myModalEditAttribute" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    	<div class="modal-dialog">
	    	<div class="modal-content">
	    		<div class="modal-header">
      				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
      				<h4 class="modal-title" id="myModalLabel">Edit Specific Attribute</h4>
			    </div>
		    	<g:form controller="specificAttribute" action="edit" method="POST" name="myFormEditAttribute" id="myFormEditAttribute">
		    		<g:hiddenField name="guidSlr" value="${slrInstance.guid}" />
		    		<g:hiddenField name="idEditAttribute" value="${idEditAttribute}" />
		    		<div class="modal-body">
				    	<g:if test="${null != errorEditAttribute && !errorEditAttribute.equals("")}">
					    	<div id="divErrorEditAttribute" class="alert alert-danger" role="alert"><i class="fa fa-remove fa-fw"></i> ${errorEditAttribute}</div>
				    	</g:if>
				    	<div class="form-inline">
				    		<table>
				    			<tr>
				    				<td><b>Name:</b></td>
				    				<td><input id="nombreEdit" type="text" name="nombreEdit" class="form-control" value="${nombreEditAttribute}" /></td>
				    			</tr>
				    			<tr>
				    				<td><b>Type:</b></td>
				    				<td>
				    					<select id="tipoEdit" name="tipoEdit" disabled="disabled">
				    			...
				    			<tr>
				    				<td><label id="lbOptionsEdit" style="visibility: hidden;"><b>Options: (;)</b></label></td>
				    				<td>
				    					<g:if test="${opcionesEditAttribute == null || opcionesEditAttribute == 'null'}">
				    						<textarea id="opcionesEdit" class="form-control" name="opcionesEdit" style="resize: none; width: 400px; height: 80px; visibility: hidden;" disabled="disabled"></textarea>
				    					</g:if>
				    					<g:else>
				    						<textarea id="opcionesEdit" class="form-control" name="opcionesEdit" style="resize: none; width: 400px; height: 80px; visibility: hidden;" disabled="disabled">${opcionesEditAttribute}</textarea>
				    					</g:else>
				    				</td>
				    			</tr>
				    		</table>
				    ...
				    <div class="modal-footer">
						<button class="btn btn-default" data-dismiss="modal" type="button">Close</button>
						<g:submitButton name="create" class="btn btn-primary" value="Edit attribute"/>
					</div>
				</g:form>
		    </div>
		</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<g:form controller="specificAttribute" action="delete" id="myFormDelete" name="myFormDelete" method="DELETE">
					...
					<g:hiddenField name="guidSlr" value="${slrInstance.guid}" />
		    		<div class="modal-body">
						The delete of this attribute will remove all attributes for every reference.
						<p> </p>
                        ...
                        <tbody>
                            <g:each in="${specAttributesListInstance}" var="attributeInstance">
                            	...
                            	<tr class="gradeX">
                            		...
                            		<td>
                            			<button title="Edit Attribute" type="button" class="btn btn-default btn-circle" data-toggle="modal" data-target="#myModalEditAttribute" onclick="getAttributeForEdit('${attributeInstance.id.toString()}','${attributeInstance.name.toString()}','${attributeInstance.tipo.toString()}','${strOptions}')"><i class="fa fa-edit"></i></button>
                            			<g:if test="${slrInstance.noDrop == false}">
											<button title="Delete Specific Attribute" type="button" class="btn btn-default btn-circle" data-toggle="modal" data-target="#myModalDrop" onclick="getIdAttribute('${attributeInstance.id.toString()}')"><i class="fa fa-times"></i></button>
										</g:if>
										<g:else>
											<button title="Delete Specific Attribute" type="button" class="btn btn-default btn-circle disabled" data-toggle="modal" data-target="#myModalDrop"><i class="fa fa-times"></i></button>
										</g:else>
                            		</td>
                            	</tr>
                            </g:each>
