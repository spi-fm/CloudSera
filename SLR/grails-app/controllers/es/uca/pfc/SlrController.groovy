...
class SlrController {
	...
	def list(){
		...
		if(springSecurityService.loggedIn){
			def usuario = UserProfile.get(springSecurityService.principal.id)
			def listOfSlr = Slr.executeQuery("FROM Slr WHERE (visibility_id=1) OR (visibility_id=2) OR (visibility_id=3) OR (visibility_id=4 AND user_profile_id=${usuario.id})")
			def droplist = []
			listOfSlr.each{
				def amigo = UserProfile.get(it.userProfileId)
				if(!(amigo in usuario.friends))
					droplist += it
			}
	...
	def edit(){
		if (springSecurityService.loggedIn){
			def userLogin = User.get(springSecurityService.principal.id)
			def slrInstance = Slr.findById(params.id)

			slrInstance.justification = params.justification
			slrInstance.objectives = params.objectives
			slrInstance.threats_to_validity = params.threats_to_validity
			slrInstance.conclusions = params.conclusions
			slrInstance.save()
			redirect(controller: 'slr', action: 'myList')
		}
	...
	def shareWithFriend(){
		if(springSecurityService.loggedIn){
			def slrInstance = Slr.findById(params.idSlr)
			def friend = UserProfile.findById(params.idUser)

			friend.addToUserProfile(slrInstance)

			redirect(controller: 'slr', action: 'index')
		}
